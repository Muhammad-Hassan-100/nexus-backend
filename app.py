...
def login():
    ...
    try:
        ...
        try:
            result = supabase.table('credentials').select('*').limit(1).execute()
        except Exception:
            try:
                default_credentials = [
                    {"username": "admin", "password": "admin123"},
                    {"username": "duet_admin", "password": "duet2024"},
                    {"username": "superuser", "password": "super123"}
                ]
                supabase.table('credentials').insert(default_credentials).execute()
            except Exception as create_error:
                print(f"Error creating credentials table: {create_error}")
        
        result = supabase.table('credentials').select('id, username').eq('username', username).eq('password', password).execute()
...
def update_credentials():
    ...
    try:
        ...
        if not data or 'current_username' not in data or 'current_password' not in data:
            return jsonify({"success": False, "message": "Current credentials are required"}), 400
        ...
        result = supabase.table('credentials').select('id').eq('username', current_username).eq('password', current_password).execute()
        
        if not result.data or len(result.data) == 0:
            return jsonify({"success": False, "message": "Invalid current credentials"}), 401
...
def get_credentials():
    ...
    try:
        ...
        result = supabase.table('credentials').select('username, password').eq('username', username).execute()
